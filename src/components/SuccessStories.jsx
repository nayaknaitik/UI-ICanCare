...
const diseases = [
  {
    ...
    image: dis1,
    icon: <img src={danger} alt="" className="h-4 md:h-12 md:mr-4" />,
  },
  {
  ...
  },
  {
    ...
    icon: <img src={ex} alt="" className="h-4 md:h-12 md:mr-4" />,
  },
  {
  ...
  return (
    <div id="know" className="max-w-7xl mx-auto font-[poppins] mt-16 my-auto">
      <div className="text-center py-12">
        ...
        <div className="flex justify-end gap-8 items-center px-8 mt-12">
          <div className="flex items-center">
            <img src={ex} alt="" className="mr-4 h-8" />
            <h4 className="md:text-xl text-sm font-medium text-gray-800">Moderate Risk</h4>
          </div>
          <div className="flex items-center">
            <img src={danger} alt="" className="mr-4 h-8" />
            <h4 className="md:text-xl font-medium text-gray-800">High Risk</h4>
          </div>
        </div>
      </div>

      <div className="grid grid-cols-3 sm:grid-cols-3 lg:grid-cols-3 gap-2 md:gap-6 px-8 font-[poppins] md:max-w-[6xl]">
        {diseases.map((disease, index) => (
          ...
          >
            {/* Front Side */}
            <div
              className={`absolute inset-0 flex flex-col p-1 md:p-4 backface-hidden rounded-xl bg-white z-10 transition-transform duration-700 ease-in-out ${
                flippedCardIndex === index ? "rotate-y-180" : ""
              } md:group-hover:rotate-y-180`}
            >
              <div className="flex items-center mb-2 h-10 sm:h-12">
                {disease.icon}
                <h3 className="text-[11px] md:text-xl lg:text-2xl font-bold text-[#2987D7] tracking-tighter ml-2 mt-2 leading-snug">
                  {disease.title}
                </h3>
              </div>
              <p className="text-gray-600 text-[7px] md:text-sm leading-relaxed  md:mt-6">
                {disease.desc}
              </p>
