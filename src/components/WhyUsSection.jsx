...
const features = [
  {
    icon: Clever,
    title: "Smart Quit",
    desc: "No willpower struggles, just a scientifically backed approach",
  },
  {
    icon: Support,
    title: "Support",
...
const WhyUsSection = () => {
  return (
    <div
      className="w-full bg-[#DAF0FF] py-8 md:py-16 px-2 md:px-8 text-center flex flex-col justify-center items-center font-[poppins]"
    >
          ...
          {features.map((item, index) => (
            ...
            >
              ...
              <div className="bg-[#A2DAFF] w-full flex items-center justify-center h-16 md:h-56">
                <img className="h-8 md:h-24" src={item.icon} alt={item.title} />
              </div>

              {/* Bottom Half - Title and Text */}
              <div className="bg-white p-2 md:p-8 flex flex-col justify-center items-center">
                <h3 className="text-[10px] md:text-2xl font-bold text-black mb-1 md:mb-2">{item.title}</h3>
                <p className="text-[6px] md:text-sm text-gray-600 font-extralight">{item.desc}</p>
              </div>
            </div>
          ))}
        </div>
